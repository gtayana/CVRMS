#!/data/lovemun/src_packages/R-3.5.1/bin/Rscript
#
#
...
## Load input datasets
library(data.table)
geno <- fread(genofile, header=T, check.names=F)
...
#		cor_mat <- cor(train_geno[,init_set][, !zv], method = "pearson")
...
#		train_ix <- init_set[!apply(cor_mat, 2, function(x) any(x > 0.9))]
	...
	ld_count <- 0
	gc()
	while (train_itr_acc < acc1 | length(train_ix) < min_num | o_delta < delta){
		if (all(length(train_ix) >= min_num, 
				any(length(train_ix) == max_num, ld_count == 20, difftime(Sys.time(), s_time, units = "hours") > time_cutoff))){
		...
		if (sel_snps == 1){
			...
			cor_mat <- cor(train_geno[,init_set][, !zv])
			highCorr <- findCorrelation(cor_mat, 0.9)
			add_snps <- init_set[-highCorr]
#			cor_mat <- cor(train_geno[,init_set], method = "pearson")
#			cor_mat[upper.tri(cor_mat)] <- 0
#			diag(cor_mat) <- 0
